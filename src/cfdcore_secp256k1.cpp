...
ByteData Secp256k1::CalculateSchnorrsigSecp256k1(
  ...
  if (oracle_privkey.GetDataSize() != 32) {
    warn(CFD_LOG_SOURCE, "Invalid Argument oracle_privkey size.");
    throw CfdException(
        CfdError::kCfdIllegalArgumentError, "Invalid oracle_privkey size.");
  }
  if (k_value.GetDataSize() != 32) {
    warn(CFD_LOG_SOURCE, "Invalid Argument k_value size.");
    throw CfdException(
        CfdError::kCfdIllegalArgumentError, "Invalid k_value size.");
  }
  const secp256k1_nonce_function nonce_func =
      (nonce_function_type == 1) ? CfdSecp256k1ConstantNonceFunction : nullptr;

  secp256k1_schnorrsig sig;
  std::vector<uint8_t> message_bytes = message.GetBytes();
  std::vector<uint8_t> oracle_bytes = oracle_privkey.GetBytes();
  std::vector<uint8_t> k_value_bytes = k_value.GetBytes();
  int ret = secp256k1_schnorrsig_sign(
      context, &sig, nonce_is_negated, message_bytes.data(),
      oracle_bytes.data(), nonce_func, k_value_bytes.data());
  if (ret != 1) {
    throw CfdException(
        CfdError::kCfdInternalError, "Secp256k1 schnorr sign error.");
  }
  // sig.data[0:31] is nonce?
  std::vector<uint8_t> result;
  result.assign(sig.data, sig.data + 64);
  return ByteData(result);
}

bool Secp256k1::VerifySchnorrsigSecp256k1(
  ...
  if (signature.GetDataSize() != 64) {
    warn(CFD_LOG_SOURCE, "Invalid Argument signature size.");
    throw CfdException(
        CfdError::kCfdIllegalArgumentError, "Invalid signature size.");
  }

  secp256k1_pubkey pubkey_data;
  std::vector<uint8_t> pubkey_bytes = pubkey.GetBytes();
  int ret = secp256k1_ec_pubkey_parse(
      context, &pubkey_data, pubkey_bytes.data(), pubkey_bytes.size());
  if (ret != 1) {
    warn(CFD_LOG_SOURCE, "Secp256k1 pubkey parse Error.");
    throw CfdException(
        CfdError::kCfdIllegalArgumentError, "Secp256k1 pubkey parse Error.");
  ...
  secp256k1_schnorrsig sig;
  memcpy(sig.data, signature.GetBytes().data(), 64);
  std::vector<uint8_t> message_bytes = message.GetBytes();
  return secp256k1_schnorrsig_verify(
             context, &sig, message_bytes.data(), &pubkey_data) == 1;
}

ByteData Secp256k1::GetSchnorrPubkeySecp256k1(
  ...
  if (oracle_pubkey.GetDataSize() != 33) {
    warn(CFD_LOG_SOURCE, "Invalid Argument oracle pubkey size.");
    throw CfdException(
        CfdError::kCfdIllegalArgumentError, "Invalid oracle pubkey size.");
  }
  if (oracle_r_point.GetDataSize() != 33) {
    warn(CFD_LOG_SOURCE, "Invalid Argument oracle r_point size.");
    throw CfdException(
        CfdError::kCfdIllegalArgumentError, "Invalid oracle r_point size.");
  }

  secp256k1_pubkey oracle_pubkey_data;
  std::vector<uint8_t> oracle_pubkey_bytes = oracle_pubkey.GetBytes();
  int ret = secp256k1_ec_pubkey_parse(
      context, &oracle_pubkey_data, oracle_pubkey_bytes.data(),
      oracle_pubkey_bytes.size());
  if (ret != 1) {
    warn(CFD_LOG_SOURCE, "Secp256k1 pubkey parse Error.");
    throw CfdException(
        CfdError::kCfdIllegalArgumentError, "Secp256k1 pubkey parse Error.");
  ...
  secp256k1_pubkey oracle_r_point_data;
  std::vector<uint8_t> oracle_r_point_bytes = oracle_r_point.GetBytes();
  ret = secp256k1_ec_pubkey_parse(
      context, &oracle_r_point_data, oracle_r_point_bytes.data(),
      oracle_r_point_bytes.size());
  if (ret != 1) {
    warn(CFD_LOG_SOURCE, "Secp256k1 pubkey parse Error.");
    throw CfdException(
        CfdError::kCfdIllegalArgumentError, "Secp256k1 pubkey parse Error.");
  ...
  std::vector<uint8_t> message_bytes = message.GetBytes();
  ret = secp256k1_schnorrsig_sig_pubkey(
      context, &result, &oracle_r_point_data, message_bytes.data(),
      &oracle_pubkey_data);
  if (ret != 1) {
    warn(CFD_LOG_SOURCE, "Secp256k1 schnorrsig pubkey Error.");
    throw CfdException(
        CfdError::kCfdIllegalArgumentError,
        "Secp256k1 schnorrsig pubkey Error.");
  }
  ...
  // Format ByteData
  ret = secp256k1_ec_pubkey_serialize(
      context, byte_data.data(), &byte_size, &result, SECP256K1_EC_COMPRESSED);
  if (ret != 1) {
    warn(CFD_LOG_SOURCE, "Secp256k1 pubkey serialize Error.");
    throw CfdException(
        CfdError::kCfdIllegalArgumentError,
...
ByteData Secp256k1::GetSchnorrPublicNonceSecp256k1(const ByteData& privkey) {
  secp256k1_context* context =
      static_cast<secp256k1_context*>(secp256k1_context_);
  if (secp256k1_context_ == NULL) {
    warn(CFD_LOG_SOURCE, "Secp256k1 context is NULL.");
  ...
  std::vector<uint8_t> privkey_bytes = privkey.GetBytes();
  memcpy(schnorrnonce.data, privkey_bytes.data(), 32);
  secp256k1_pubkey pubnonce;
  int ret =
      secp256k1_schnorrsig_get_public_nonce(context, &pubnonce, &schnorrnonce);
  if (ret != 1) {
    warn(CFD_LOG_SOURCE, "Secp256k1 schnorrsig pubkey Error.");
    throw CfdException(
        CfdError::kCfdIllegalArgumentError,
  ...
  size_t byte_size = byte_data.size();
  ret = secp256k1_ec_pubkey_serialize(
      context, byte_data.data(), &byte_size, &pubnonce,
      SECP256K1_EC_COMPRESSED);
  if (ret != 1) {
    warn(CFD_LOG_SOURCE, "Secp256k1 pubkey serialize Error.");
    throw CfdException(
